...
impl Repr {
    pub(super) fn new(dat: ErrorData<Box<Custom>>) -> Self {
        match dat {
            ...
            ErrorData::SgxStatus(status) => Self::new_sgx(status),
        }
    }
    ...
    #[inline]
    pub(super) fn new_sgx(status: SgxStatus) -> Self {
        let utagged = ((status as usize) << 32) | TAG_SGX;
        // Safety: `TAG_OS` is not zero, so the result of the `|` is not 0.
        let res = Self(unsafe { NonNull::new_unchecked(ptr::invalid_mut(utagged)) }, PhantomData);
        // quickly smoke-check we encoded the right thing (This generally will
        // only run in std's tests, unless the user uses -Zbuild-std)
        debug_assert!(
            matches!(res.data(), ErrorData::SgxStatus(s) if s == status),
            "repr(sgx) encoding failed for {status}"
        );
        res
    }
    ...
    F: FnOnce(*mut Custom) -> C,
{
    ...
    match bits & TAG_MASK {
        ...
        TAG_SGX => {
            let status = ((bits as i64) >> 32) as u32 as SgxStatus;
...
static_assert!(@usize_eq: TAG_MASK & TAG_SGX, TAG_SGX);
