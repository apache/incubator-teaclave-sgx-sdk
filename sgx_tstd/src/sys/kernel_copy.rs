...
#[cfg(feature = "stdio")]
use crate::io::{StderrLock, StdinLock, StdoutLock};
...
pub(super) fn copy_regular_files(reader: RawFd, writer: RawFd, max_len: u64) -> CopyResult {
    ...
    match HAS_COPY_FILE_RANGE.load(Ordering::Relaxed) {
        NOT_PROBED => {
            ...
            let result = unsafe {
                cvt_ocall(libc::copy_file_range(INVALID_FD, None, INVALID_FD, None, 1, 0))
            };
    ...
    while written < max_len {
        ...
        let copy_result = unsafe {
            ...
            cvt_ocall(libc::copy_file_range(reader, None, writer, None, bytes_to_copy, 0))
        };
...
fn sendfile_splice(mode: SpliceMode, reader: RawFd, writer: RawFd, len: u64) -> CopyResult {
    ...
    while written < len {
        ...
        let result = match mode {
            SpliceMode::Sendfile => {
                cvt_ocall(unsafe { libc::sendfile(writer, reader, None, chunk_size) })
            }
            SpliceMode::Splice => cvt_ocall(unsafe {
                libc::splice(reader, None, writer, None, chunk_size, 0)
            }),
        };
...
mod libc {
    pub use sgx_oc::ocall::{copy_file_range, sendfile, splice};
    pub use sgx_oc::*;
}
