...
/// `derive_serialize` provides the `Serialize` macro for `sgx_serialize
///
...
/// value.
#[proc_macro_derive(Serialize, attributes(sgx_serialize))]
...
/// `derive_deserialize` provides the `Deserialize` macro for `sgx_serialize
///
/// `derive_deserialize` takes one parameter typed `TokenStream` and parse the
/// input stream brought by it. Then expand the parsed the result as return
/// value.
#[proc_macro_derive(Deserialize, attributes(sgx_serialize))]
