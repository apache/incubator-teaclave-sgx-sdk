...
impl Stats for [f64] {
    ...
    fn median_abs_dev(&self) -> f64 {
        let med = self.median();
        let abs_devs: Vec<f64> = self.iter().map(|&v| abs(med - v)).collect();
...
#[inline]
fn sqrt(f: f64) -> f64 {
    unsafe { intrinsics::sqrtf64(f) }
}

#[inline]
fn abs(f: f64) -> f64 {
    unsafe { intrinsics::fabsf64(f) }
}

#[inline]
fn floor(f: f64) -> f64 {
    unsafe { intrinsics::floorf64(f) }
}
