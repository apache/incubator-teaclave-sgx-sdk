...
extern "C" {
    #[allow(dead_code)]
    pub fn ocall_get_update_info (ret_val: *mut SgxStatus,
                                  platformBlob: * const PlatformInfo,
                                  enclaveTrusted: i32,
...
pub fn gen_ecc_cert(payload: String,
                    prv_k: &EcPrivateKey,
                    pub_k: &EcPublicKey,
                    ecc_handle: &EcKeyPair) -> Result<(Vec<u8>, Vec<u8>), SgxStatus> {
...
pub fn verify_mra_cert(cert_der: &[u8]) -> Result<(), SgxStatus> {
        ...
        match quote_status.as_ref() {
            ...
            "GROUP_OUT_OF_DATE" | "GROUP_REVOKED" | "CONFIGURATION_NEEDED" => {
                ...
                if let Value::String(pib) = &attn_report["platformInfoBlob"] {
                    ...
                    // let res = unsafe{
                    //     ocall_get_update_info(&mut rt as *mut SgxStatus,
                    //                           platform_info.as_slice().as_ptr() as * const PlatformInfo,
                    //                           1,
                    //                           &mut update_info as * mut UpdateInfoBit)
                    // };
                    // if res != SgxStatus::SGX_SUCCESS {
                    //     println!("res={:?}", res);
                    //     return Err(res);
                    // }

                    // if rt != SgxStatus::SGX_SUCCESS {
                    //     println!("rt={:?}", rt);
