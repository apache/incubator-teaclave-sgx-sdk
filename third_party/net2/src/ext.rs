...
#[cfg(any(unix, target_os = "redox"))] use sgx_trts::libc::*;
...
pub fn set_opt<T: Copy>(sock: Socket, opt: c_int, val: c_int,
    ...
    unsafe {
        ...
        try!(::cvt(c::setsockopt(sock, opt, val, payload as *const _,
...
pub fn get_opt<T: Copy>(sock: Socket, opt: c_int, val: c_int) -> io::Result<T> {
    unsafe {
        ...
        #[cfg(target_os = "redox")]
        let sock = sock as c_int;
        try!(::cvt(c::getsockopt(sock, opt, val,
...
cfg_if! {
    if #[cfg(any(target_os = "macos", target_os = "ios"))] {
        ...
        use sgx_trts::libc::TCP_KEEPIDLE as KEEPALIVE_OPTION;
...
impl UdpSocketExt for UdpSocket {
    ...
    fn send(&self, buf: &[u8]) -> io::Result<usize> {
        unsafe {
            ::cvt(c::send(self.as_sock() as c_int, buf.as_ptr() as *const _, buf.len(), 0)).map(|n| n as usize)
        }
    }
    ...
    fn recv(&self, buf: &mut [u8]) -> io::Result<usize> {
        unsafe {
            ::cvt(c::recv(self.as_sock(), buf.as_mut_ptr() as *mut _, buf.len(), 0))
                .map(|n| n as usize)
        }
    }
...
fn set_nonblocking(sock: Socket, nonblocking: bool) -> io::Result<()> {
    ...
    ::cvt(unsafe {
        c::ioctl(sock, FIONBIO, &mut nonblocking)
    }).map(|_| ())
}
